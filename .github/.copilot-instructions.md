...
### Authentication Flow
**Cookie-based authentication with secure HTTP cookies (NO Authorization headers or localStorage)**
1. Magic link email authentication (no passwords)
2. JWT access tokens (15 min) + refresh tokens (30 days) stored as HttpOnly cookies
3. `AuthMiddleware` validates tokens from cookies only
4. User context passed through request context

**Complete authentication documentation**: See `/Backend/AUTHORIZATION_FLOW.md` for detailed flow specifications.
