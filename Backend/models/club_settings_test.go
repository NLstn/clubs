...
func TestClubSettingsCreatedWithClub(t *testing.T) {
	...
	t.Run("settings created automatically when club is created via OData", func(t *testing.T) {
		...
		assert.False(t, settings.NewsEnabled, "NewsEnabled should be false by default")
		assert.False(t, settings.EventsEnabled, "EventsEnabled should be false by default")
	...
	t.Run("test helper creates settings with correct defaults", func(t *testing.T) {
		...
		assert.False(t, settings.NewsEnabled)
		assert.False(t, settings.EventsEnabled)
